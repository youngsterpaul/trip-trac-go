...
  if (loading || !place) return <div className="min-h-screen bg-background"><div className="hidden sm:block"><Header /></div><div className="h-96 bg-muted animate-pulse" /><MobileBottomBar /></div>;

  const displayImages = [place.image_url, ...(place.gallery_images || []), ...(place.images || [])].filter(Boolean);

  return (
    <div className="min-h-screen bg-background pb-20 md:pb-0">
      {/* Hide Header on small screen (max-width 640px) */}
      <div className="hidden sm:block">
        <Header />
      </div>

      <main className="container px-4 py-6 sm:py-0 max-w-6xl mx-auto sm:p-0">
        
        {/* The main grid layout */}
        <div className="grid lg:grid-cols-[2fr,1fr] gap-6 sm:gap-4">
          <div 
            className="w-full relative"
            // Ensure margin is zero on small screens
            style={{ margin: 0 }} 
          >
            {/* Back Button for ALL screens, positioned over the image slideshow */}
            <Button 
              variant="ghost" 
              onClick={() => navigate(-1)} 
              className="absolute top-4 left-4 z-20 w-10 h-10 rounded-full flex items-center justify-center" // Removed sm:hidden
              style={{ backgroundColor: 'rgba(0, 0, 0, 0.4)', color: 'white' }} // RGBA color applied
            >
              <ArrowLeft className="h-5 w-5" />
            </Button>

            {/* Image Carousel - Removed all rounded-* classes to ensure zero border radius on all screens */}
            <Carousel opts={{ loop: true }} plugins={[Autoplay({ delay: 3000 })]} className="w-full overflow-hidden rounded-none">
              <CarouselContent>
...
            </Carousel>
            
            {/* START: Description Section - Removed rounded-b-2xl and sm:rounded-b-2xl for zero border radius */}
            {place.description && (
              <div 
...
          </div>
          
          {/* Content Details - Adjusted padding to account for full-width image on small screens */}
          <div className="px-4 sm:px-4 lg:px-0 space-y-4 sm:space-y-3 pt-6 sm:pt-4">
            <div>
...
          <div className="mt-6 sm:mt-4 p-6 sm:p-3 border bg-card sm:mx-4">
